...
class TicTacToeAI:
    ...
    def isWinningMove(tacToCheck, board, position):
        """
        A function that detects whether or not a given position with a given
        tac makes a win on a given board.
        """
        testBoard = TicTacToeBoard()
        testBoard.TTTBoard = board.TTTBoard[:]
        if testBoard.putTac(tacToCheck, position):
    ...
    # Checks for winning moves for the supplied tac, then places the self tac
    # at a winning move if possible
    def placeAtWinningMove(self, board, tacToCheck):
        """
        Instance method using isWinningMove to have the AI place a tac
        appropriately. Can be used for blocking enemy wins or making a winning
        move.
        """
        for i in range (1, 10):
